  ...
  return <div style={{animation:"up .3s ease"}}>
    ...
    <div className="gm">
      {fil.map((m,i)=>{const f=pr[m.id]==="done",s=sc[m.id],lk=!ok;return <div key={m.id} onClick={()=>lk?onSub():onOpen(m)} className="hv" style={{background:K.card,border:`1px solid ${f?m.col+"44":K.b0}`,borderRadius:11,padding:"12px",cursor:"pointer",position:"relative",overflow:"hidden",animation:`up .3s ease ${i*15}ms both`,opacity:lk?.65:1}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}><div style={{width:28,height:28,borderRadius:8,background:f?`${m.col}18`:"rgba(128,128,128,.08)",border:`1px solid ${f?m.col+"33":K.b0}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{lk?"🔒":m.ico}</div>{f&&s&&<Tg c={s.pct>=60?K.em:K.er} bg={s.pct>=60?K.emBg:K.erBg} bd={s.pct>=60?K.emBd:K.erBd} ch={`${s.pct}%`}/>}</div>
    ...
    {ex&&<div style={{background:K.card,border:`1px solid ${exO?K.inBd:K.b0}`,borderRadius:12,marginBottom:10,overflow:"hidden"}}>
      ...
      {exO&&<div style={{borderTop:`1px solid ${K.b0}`,padding:"13px"}}>
        {!ok?<div style={{textAlign:"center",padding:"10px 0"}}><div style={{fontSize:22,marginBottom:5}}>🔒</div><div style={{color:K.t3,fontSize:13}}>Abonnés actifs uniquement.</div></div>:<>
          ...
          {ex.dn?.length>0&&<div style={{marginBottom:10}}><div style={{color:K.t3,fontSize:10,fontWeight:700,letterSpacing:"1.2px",textTransform:"uppercase",fontFamily:"'JetBrains Mono',monospace",marginBottom:5}}>Données</div><div style={{background:K.bg,border:`1px solid ${K.b0}`,borderRadius:8,overflow:"hidden"}}>{ex.dn.map((row,i)=><div key={i} style={{display:"flex",padding:"7px 11px",borderBottom:i<ex.dn.length-1?`1px solid ${K.b0}`:"none",gap:9}}>{(Array.isArray(row)?row:[row]).map((v,j)=><span key={j} style={{color:j===0?K.t2:K.t1,fontFamily:j>0?"'JetBrains Mono',monospace":"'Outfit',sans-serif",fontSize:j>0?12:13,flex:j===0?1:"none",fontWeight:j>0?600:400}}>{v}</span>)}</div>)}</div></div>}
  ...
  function ModEditor({mod,onSave,onCancel}){
    ...
    return <div style={{animation:"sc .2s ease"}}>
      ...
      <div style={{background:K.c2,borderRadius:11,padding:"13px",marginBottom:12}}>
        ...
        <div style={{marginBottom:8}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:5}}><div style={{color:K.t2,fontSize:12,fontWeight:600}}>Données</div><Btn ch="+ Ligne" on={()=>sEx(x=>({...x,dn:[...x.dn,["",""]]}))} sm v="s"/></div>{ex.dn.map((row,i)=><div key={i} style={{display:"flex",gap:4,marginBottom:3,alignItems:"center"}}>{row.map((v,j)=><input key={j} value={v} onChange={e=>{const dn=[...ex.dn];dn[i]=[...dn[i]];dn[i][j]=e.target.value;sEx(x=>({...x,dn}));}} placeholder={j===0?"Libellé":"Valeur"} style={{flex:1,padding:"6px 8px",background:K.bg,border:`1px solid ${K.b0}`,borderRadius:6,color:K.t1,fontSize:11,outline:"none",fontFamily:j>0?"'JetBrains Mono',monospace":"'Outfit',sans-serif"}}/>)}<Btn ch="✕" on={()=>sEx(x=>({...x,dn:x.dn.filter((_,j)=>j!==i)}))} v="d" sm/></div>)}</div>
      ...
      {tab==="m"&&<div style={{animation:"up .25s ease"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {mods.map(m=><div key={m.id} style={{background:K.card,border:`1px solid ${m.on!==false?K.b1:K.b0}`,borderRadius:11,padding:"11px 13px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",opacity:m.on!==false?1:.6}}>
            <div style={{width:34,height:34,borderRadius:9,background:`${m.col}18`,border:`1px solid ${m.col}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0}}>{m.ico}</div>
