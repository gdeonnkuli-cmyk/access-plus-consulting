...
const loadJsPDF=()=>new Promise((res,rej)=>{
  ...
  sc.onerror=()=>rej(new Error("Impossible de charger le générateur PDF. Vérifiez votre connexion."));
  document.head.appendChild(sc);
});
const genAttestation=async({nom,mods,pr,sc})=>{
  const jsPDF=await loadJsPDF();
  const done=mods.filter(m=>pr[m.id]==="done");
  const vals=done.map(m=>sc[m.id]?.pct).filter(p=>p!=null);
  const avg=vals.length?Math.round(vals.reduce((a,b)=>a+b,0)/vals.length):null;
  const d=new jsPDF({orientation:"landscape",unit:"mm",format:"a4"});
  d.setDrawColor(26,107,58);d.setLineWidth(1.5);d.rect(8,8,281,194);
  d.setDrawColor(218,168,84);d.setLineWidth(.5);d.rect(11,11,275,188);
  d.setTextColor(26,107,58);d.setFont("times","bold");d.setFontSize(30);
  d.text("ATTESTATION DE FORMATION",148.5,42,{align:"center"});
  d.setDrawColor(218,168,84);d.setLineWidth(.8);d.line(98,48,199,48);
  d.setTextColor(80,80,80);d.setFont("helvetica","normal");d.setFontSize(13);
  d.text("Éco-Campus RDC certifie que",148.5,64,{align:"center"});
  d.setFont("times","bolditalic");d.setFontSize(26);d.setTextColor(20,20,20);
  d.text(nom||"—",148.5,80,{align:"center"});
  d.setFont("helvetica","normal");d.setFontSize(12);d.setTextColor(80,80,80);
  d.text(`a suivi avec succès ${done.length} module${done.length>1?"s":""} de formation en comptabilité SYSCOHADA Révisé`,148.5,93,{align:"center"});
  if(avg!=null)d.text(`avec un score moyen de ${avg} %`,148.5,101,{align:"center"});
  d.setFontSize(9);d.setTextColor(120,120,120);
  done.slice(0,6).forEach((m,i)=>d.text(`• ${m.titre}`,148.5,113+i*5.5,{align:"center"}));
  if(done.length>6)d.text(`… et ${done.length-6} autre${done.length-6>1?"s":""} module${done.length-6>1?"s":""}`,148.5,113+6*5.5,{align:"center"});
  d.setFontSize(11);d.setTextColor(80,80,80);
  d.text(`Fait à Kinshasa, le ${new Date().toLocaleDateString("fr-FR")}`,148.5,168,{align:"center"});
  d.setFont("helvetica","bold");d.setTextColor(26,107,58);
  d.text("Éco-Campus RDC · eco-campus-rdc.vercel.app · +243 813 165 403",148.5,180,{align:"center"});
  d.save(`attestation-${(nom||"apprenant").replace(/\s+/g,"-").toLowerCase()}.pdf`);
};
...
function UA({uid,onOut}){
  ...
  return W(<>
    ...
    {vue==="res"&&<Res sc={sc} ok={ok} mods={aMods} u={uData} pr={pr} nd={nd} gp={gp}/>}
...
function PsychoPage({uid,u}){
  ...
  if(phase==="level")return <div style={{padding:mob?"14px":"24px 0",animation:"fadeIn .35s ease"}}>
    <div style={{textAlign:"center",marginBottom:22}}>
      ...
      <div style={{fontSize:13,color:K.t2,lineHeight:1.6,maxWidth:460,margin:"0 auto"}}>Choisissez votre niveau · 9 catégories : suites, logique, figures, dominos, calcul, français, anglais, attention… · QCM et réponses libres · 5 à 15 minutes</div>
    </div>
...
function Nav({u,vue,sV,ok,onSub,onOut,live,uid}){
  ...
  if(mob){
    ...
    return <>
      ...
      {/* Bottom nav */}
      <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:99,background:`${K.card}f5`,backdropFilter:"blur(20px)",borderTop:`1px solid ${K.b0}`,display:"flex",alignItems:"center",overflowX:"auto",WebkitOverflowScrolling:"touch",paddingBottom:"max(6px,env(safe-area-inset-bottom))"}}>
        {visible.map(({k,ico,label,live:isLive,badgeCount})=>{
          const active=vue===k;
          const hasLive=isLive&&live;
          return <button key={k} onClick={()=>sV(k)} className="bt" style={{flex:"1 0 auto",minWidth:62,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,padding:"8px 6px 4px",background:"none",border:"none",cursor:"pointer",position:"relative",minHeight:52}}>
...
function Res({sc,ok,mods,u,pr,nd,gp}){
  const[attBusy,sAttBusy]=useState(false);
  ...
  const attReady=gp===100&&nd>0;
  return <div style={{maxWidth:700,margin:"0 auto",animation:"up .25s ease"}}>
    {ok&&<div style={{background:K.card,border:`1px solid ${attReady?K.emBd:K.b0}`,borderRadius:14,padding:"15px 17px",marginBottom:14,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
      <div style={{width:40,height:40,borderRadius:11,background:attReady?K.emBg:K.c2,border:`1px solid ${attReady?K.emBd:K.b0}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
        <i className="ti ti-certificate" style={{fontSize:19,color:attReady?K.em:K.t3}}/>
      </div>
      <div style={{flex:1,minWidth:150}}>
        <div style={{fontWeight:800,fontSize:14,color:K.t1}}>Attestation de formation</div>
        <div style={{fontSize:12,color:K.t3}}>{attReady?"Félicitations ! Votre attestation est prête.":`Terminez tous les modules pour la débloquer (${nd}/${mods.length}).`}</div>
      </div>
      <Btn ch={attBusy?"Génération…":"📜 Télécharger (PDF)"} dis={!attReady||attBusy} on={async()=>{sAttBusy(true);try{await genAttestation({nom:u?.nom,mods,pr,sc});}catch(e){alert(e.message);}sAttBusy(false);}} v={attReady?"p":"g"} sm/>
    </div>}
    {/* Header */}
...
function AA({onOut}){
  ...
  const[plDate,sPlDate]=useState(new Date().toISOString().slice(0,10));
  const[plAll,sPlAll]=useState([]);
  useEffect(()=>{
    const unsub=onSnapshot(collection(db,"presences"),(snap)=>{sPlAll(snap.docs.map(d=>({id:d.id,...d.data()})));});
    return unsub;
  },[]);
    ...
    <nav className="nb" style={{background:`${K.card}f2`,backdropFilter:"blur(18px)",borderBottom:`1px solid ${K.b0}`,position:"sticky",top:0,zIndex:99}}>
      ...
      {(()=>{
        const ADMIN_TABS=[
          ...
          {k:"pl", ico:"ti-calendar-check",   label:"Présentiel",  bc:K.em},
    ...
    <div className="mp" style={{maxWidth:1060,margin:"0 auto"}}>
      ...
      {tab==="home"&&(()=>{
        const dashTiles=[
          ...
          {k:"pl",ico:"calendar-check", label:"Présentiel",     col:K.em},
        ...
        return <div style={{animation:"fadeIn .35s ease"}}>
          ...
          <KpiGroup title="Utilisateurs">
            ...
            <KpiCard label="Nouveaux ce mois" val={(()=>{const now=new Date();return users.filter(u=>{const p=String(u.createdAt||"").split("/");if(p.length!==3)return false;return +p[1]===now.getMonth()+1&&+p[2]===now.getFullYear();}).length;})()} col={K.em}/>
...
{tab==="fo"&&<div style={{animation:"fadeIn .35s ease"}}>
    ...
    {tab==="pl"&&(()=>{
      const apprs=users.filter(u=>u.role!=="formateur"&&u.mail!==ADM_EMAIL);
      const day=plAll.find(p=>p.id===plDate);
      const present=day?.present||[];
      const toggle=async uid=>{
        const next=present.includes(uid)?present.filter(x=>x!==uid):[...present,uid];
        await setDoc(doc(db,"presences",plDate),{date:plDate,present:next,updatedAt:Date.now()},{merge:true});
      };
      return <div style={{animation:"fadeIn .35s ease"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
          <div style={{fontWeight:800,fontSize:14,color:K.t1,display:"flex",alignItems:"center",gap:7}}>
            <i className="ti ti-calendar-check" style={{fontSize:16,color:K.em}}/>Présence en présentiel
          </div>
          <input type="date" value={plDate} onChange={e=>sPlDate(e.target.value)} style={{background:K.c2,border:`1px solid ${K.b1}`,borderRadius:9,padding:"8px 11px",color:K.t1,fontFamily:"'Outfit',sans-serif",fontSize:13}}/>
        </div>
        <div style={{fontSize:12,color:K.t3,marginBottom:12}}>{present.length} présent{present.length>1?"s":""} le {plDate.split("-").reverse().join("/")} · cochez pour marquer la présence</div>
        <div style={{display:"flex",flexDirection:"column",gap:7}}>
          {apprs.map(u=>{
            const isP=present.includes(u.uid);
            const total=plAll.filter(p=>p.present?.includes(u.uid)).length;
            return <div key={u.uid} onClick={()=>toggle(u.uid)} className="bt" style={{background:K.card,border:`1px solid ${isP?K.emBd:K.b0}`,borderRadius:11,padding:"11px 14px",cursor:"pointer",display:"flex",alignItems:"center",gap:11}}>
              <div style={{width:22,height:22,borderRadius:6,border:`2px solid ${isP?K.em:K.b1}`,background:isP?K.em:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                {isP&&<i className="ti ti-check" style={{fontSize:13,color:"#F5EDD8"}}/>}
              </div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:700,fontSize:13,color:K.t1}}>{u.nom}</div>
                <div style={{fontSize:11,color:K.t3,fontFamily:"'JetBrains Mono',monospace"}}>{u.mail}</div>
              </div>
              <span style={{fontSize:10,color:K.t3,flexShrink:0}}>{total} séance{total>1?"s":""} au total</span>
            </div>;
          })}
          {!apprs.length&&<div style={{background:K.c2,border:`1px solid ${K.b0}`,borderRadius:10,padding:"20px",textAlign:"center",fontSize:13,color:K.t3}}>Aucun apprenant inscrit.</div>}
        </div>
      </div>;
    })()}
