...
function UA({uid,onOut}){
  ...
  if(mod)return W(<MV mod={mod} sc={sc[mod.id]} ok={ok} onQ={()=>sQ(mod)} onOpenQ={()=>sOpenQuiz(mod)} onMarkDone={()=>markDoneNoEval(mod.id)} doneNoEval={pr[mod.id]==="done"} openAnswer={myOpenAnswers.find(a=>a.modId===mod.id)} onBack={()=>sM(null)} onSub={()=>sSub(true)} vids={vids.filter(v=>v.mid===mod.id)} pdf={pdfs[mod.id]}/>,true);
  return W(<>
...
function MV({mod,sc,ok,onQ,onOpenQ,onMarkDone,doneNoEval,openAnswer,onBack,onSub,vids,pdf}){
  const K=useK();const{mob}=useW();
    ...
    <div className="mp" style={{maxWidth:1060,margin:"0 auto"}}>
      ...
      {tab==="home"&&(()=>{
        const dashTiles=[
          ...
          {k:"msg",ico:"message-circle",label:"Messages",       badge:allMessages.filter(m=>m.fromRole==="apprenant"&&!m.luAdmin).length, col:K.rd},
        ];
        ...
        const nMsgNonLus=allMessages.filter(m=>m.fromRole==="apprenant"&&!m.luAdmin).length;
