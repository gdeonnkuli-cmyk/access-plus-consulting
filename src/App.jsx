  ...
  return <div style={{animation:"fadeIn .35s ease"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {pres.map(p=>{
        ...
        return <div key={p.id} style={{background:K.card,border:`1px solid ${K.b0}`,borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12}}>
        ...
        return <div key={q.id} style={{background:K.card,border:`1px solid ${good?K.emBd:K.rdBd}`,borderRadius:12,padding:"14px 16px"}}>
          ...
          <div style={{fontSize:12,color:K.t2,paddingLeft:28,lineHeight:1.6}}>
            {given==null
              ...
              :!good&&<div>Votre réponse : <span style={{color:K.rd,fontWeight:700}}>{q.opts[given]}</span></div>}
            <div>Bonne réponse : <span style={{color:K.em,fontWeight:700}}>{q.opts[q.a]}</span></div>
...
function AA({onOut}){
  ...
  const mL=async u=>{
    ...
    if(ok)sMsg({t:"o",m:`✅ Email envoyé à ${u.mail}`});else sMsg({t:"e",m:"❌ Échec de l'envoi. Réessayez."});
    setTimeout(()=>sMsg({t:"",m:""}),3000);
  };
...
{tab==="fo"&&<div style={{animation:"fadeIn .35s ease"}}>
        ...
        return <div style={{marginBottom:20}}>
          ...
          {pendingPres.map(p=><div key={p.id} style={{background:K.card,border:`1px solid ${K.b0}`,borderRadius:12,padding:"13px 15px",marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              <button onClick={()=>rejectContent("pres",p.id)} className="bt"
                style={{flex:1,background:K.erBg,border:`1px solid ${K.erBd}`,color:K.er,borderRadius:8,padding:"7px",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"'Outfit',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                <i className="ti ti-x" style={{fontSize:12}}/>Rejeter
              </button>
            </div>
        ...
        {lastFoCode.code&&<div style={{background:"#8B5CF618",border:"1px solid #8B5CF640",borderRadius:10,padding:"14px",textAlign:"center"}}>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
            ...
            <button onClick={()=>{
              ...
              if(u)window.open(`mailto:${u.mail}?subject=Code formateur Éco-Campus&body=Bonjour ${u.nom},%0A%0AVotre code formateur : ${lastFoCode.code}%0A%0A1. Connectez-vous sur access-plus-consulting.vercel.app%0A2. Cliquez "J'ai un code"%0A3. Saisissez : ${lastFoCode.code}%0A%0AÉco-Campus RDC`,"_blank");
            }} className="bt"
    ...
    {tab==="au"&&<div style={{animation:"fadeIn .35s ease"}}>
      ...
      <div style={{fontSize:12,color:K.t3,marginBottom:13}}>{users.length} compte{users.length>1?"s":""} · {users.filter(u=>u.lastLogin).length} déjà connecté{users.filter(u=>u.lastLogin).length>1?"s":""}</div>
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
